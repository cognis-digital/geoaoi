...
How **geoaoi** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `geoaoi` lives in the **Geospatial / GEOINT** cluster.
...
| from | relation | to |
|---|---|---|
| `geoaoi` | JSON findings compose with | [`locateanything`](https://github.com/cognis-digital/locateanything), [`geolens`](https://github.com/cognis-digital/geolens), [`geoaoi-pro`](https://github.com/cognis-digital/geoaoi-pro) |
| `geoaoi` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `geoaoi` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `geoaoi` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the geospatial / geoint cluster.** `geoaoi` output feeds the next tool:
```bash
geoaoi ... --format json > out.json          # this tool's findings (see `geoaoi --help`)
...
geoaoi ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
geoaoi ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
geoaoi ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `geoaoi`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `geoaoi` slots
